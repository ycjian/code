...
const pages = [
  {
    ...
    summary: "A Chinese-first, implementation-heavy guide covering 225 PyTorch exercises and course lessons across tensors, autograd, profiling, data loading, optimization, distributed training, custom kernels, LLM systems, and interview deep dives.",
    ...
    tabGroups: [
      ...
      { title: "课程：成本与规模", from: "103. 训练成本与规模 | 1_plot_cost_time.py" },
      { title: "课程：张量 / Autograd", from: "104. 张量语义与 Autograd | 1_tensor_creation.py" },
      { title: "课程：Profiler", from: "121. 可靠计时与性能分析 | 1_ps.sh" },
      { title: "课程：DataLoader", from: "145. 数据管线与 DataLoader | 0_cifar.sh" },
      { title: "课程：计算优化", from: "156. 计算优化 | 1_num_worker.py" },
      { title: "课程：CUDA 内存", from: "170. CUDA 内存生命周期 | 1_max_split.sh" },
      { title: "课程：DDP", from: "192. 分布式训练与 DDP | 1_single_gpu.py" },
      { title: "课程：编译 / 自定义算子", from: "197. AMP、torch.compile 与自定义算子 | 1_amp_perf_cont.py" },
      { title: "课程：miniGPT", from: "218. miniGPT 端到端训练 | 1_tensorboard.py" },
    ],
  },
  {
    input: "mlQuestion.md",
  ...
  {
    input: "ai_agent_course.md",
    output: "ai_agent_course.html",
    title: "AI Agent Harness 中文精读",
    eyebrow: "Vercel Academy / Tools / Memory / Skills",
    summary: "A polished Chinese reading version of the Vercel Academy Build Your Own AI Coding Agent Harness course, preserving code blocks, shell commands, tool logic, verification flow, memory design, subagents, and extension patterns.",
    logo: "how_assets_small/logos/ai-eng-logo.svg",
    logoAlt: "AI Agent Harness course logo",
    parentNavOnly: true,
    navTitle: "课程章节",
    pythonReaderStyle: true,
    codeFocus: true,
    ...
    collapsibleSidebar: true,
    sidebarDefault: "expanded",
    sidebarStorageKey: "interview-reader-sidebar:ai-agent-course:v1",
  },
  {
    input: "ai_eng.md",
    output: "ai_eng.html",
...
function template(page, parsed) {
  ...
  const bodyClasses = [
    page.pythonReaderStyle ? "python-reader" : "",
    page.collapsibleSidebar ? "collapsible-sidebar" : "",
    page.codeFocus ? "code-focus" : "",
  ].filter(Boolean);
...
.python-reader.code-focus{--accent:#ee4c2c;--accent2:#ff8a55}.python-reader.code-focus .brand h1{background:linear-gradient(135deg,#ffb08d,#ff6b3d);-webkit-background-clip:text;-webkit-text-fill-color:transparent}.python-reader.code-focus .tab-btn.active{border-left-color:#ee4c2c;background:rgba(238,76,44,.11);color:#ff9b75}.python-reader.code-focus .code-shell{margin:24px 0;border:1px solid rgba(255,138,85,.42);border-left:4px solid #ee4c2c;border-radius:12px;background:linear-gradient(145deg,#09111e,#0d1726 58%,#101a29);box-shadow:0 14px 34px rgba(0,0,0,.32),0 0 0 1px rgba(255,255,255,.025) inset}.python-reader.code-focus .code-title{padding:10px 17px;background:linear-gradient(90deg,rgba(238,76,44,.19),rgba(255,138,85,.07) 54%,rgba(255,255,255,.025));border-bottom-color:rgba(255,138,85,.22);color:#ffb08d;font-size:12px;font-weight:900;letter-spacing:.09em;text-transform:uppercase}.python-reader.code-focus .copy-btn{border:1px solid rgba(255,176,141,.28);border-radius:999px;padding:5px 11px;background:rgba(238,76,44,.08);color:#ffd4c2;font-size:11px;font-weight:800;letter-spacing:.02em;text-transform:none}.python-reader.code-focus .copy-btn:hover{background:#ee4c2c;color:#fff}.python-reader.code-focus pre{padding:20px 22px;color:#e6edf7;font-size:15px;line-height:1.68;font-weight:500}.python-reader.code-focus pre code.hljs{color:#e6edf7}.python-reader.code-focus .hljs-keyword,.python-reader.code-focus .hljs-selector-tag,.python-reader.code-focus .hljs-literal,.python-reader.code-focus .pseudo-key{color:#7dd3fc;font-weight:850}.python-reader.code-focus .hljs-built_in,.python-reader.code-focus .hljs-type,.python-reader.code-focus .hljs-title.class_,.python-reader.code-focus .pseudo-fn{color:#6ee7b7}.python-reader.code-focus .hljs-title,.python-reader.code-focus .hljs-title.function_,.python-reader.code-focus .hljs-function{color:#fde68a;font-weight:750}.python-reader.code-focus .hljs-string,.python-reader.code-focus .hljs-regexp,.python-reader.code-focus .hljs-symbol,.python-reader.code-focus .hljs-bullet{color:#bef264}.python-reader.code-focus .hljs-number,.python-reader.code-focus .pseudo-num{color:#ff9b75}.python-reader.code-focus .hljs-comment,.python-reader.code-focus .hljs-quote,.python-reader.code-focus .plain-code-highlight .pseudo-comment{color:#94a3b8;font-style:italic}.python-reader.code-focus .hljs-attr,.python-reader.code-focus .hljs-attribute,.python-reader.code-focus .hljs-variable,.python-reader.code-focus .hljs-template-variable,.python-reader.code-focus .pseudo-shape{color:#c4b5fd}.python-reader.code-focus .hljs-meta,.python-reader.code-focus .hljs-doctag{color:#fdba74}.python-reader.code-focus .hljs-operator,.python-reader.code-focus .hljs-punctuation,.python-reader.code-focus .pseudo-op{color:#a5b4fc}@media(max-width:560px){.python-reader.code-focus pre{padding:16px 15px;font-size:13.5px;line-height:1.62}.python-reader.code-focus .code-title{padding:9px 13px}}
