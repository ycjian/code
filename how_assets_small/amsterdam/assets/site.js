...
els.frame?.addEventListener("load", installDocumentNextButton);
...
function installDocumentNextButton() {
  const frameDocument = els.frame?.contentDocument;
  if (!frameDocument?.body || frameDocument.querySelector(".hub-page-next")) return;
  const currentIndex = state.manifest.documents.findIndex(doc => doc.id === state.activeDocId);
  const nextDocument = state.manifest.documents[currentIndex + 1];
  const nav = frameDocument.createElement("div");
  nav.className = "hub-page-next-wrap";
  const button = frameDocument.createElement("button");
  button.type = "button";
  button.className = "hub-page-next";
  button.textContent = "Next →";
  button.disabled = !nextDocument;
  if (nextDocument) button.addEventListener("click", () => openDocument(nextDocument.id));
  nav.appendChild(button);
  frameDocument.body.appendChild(nav);

  const style = frameDocument.createElement("style");
  style.textContent = ".hub-page-next-wrap{display:flex;justify-content:flex-end;width:min(1240px,calc(100% - 28px));margin:24px auto 36px;padding-top:18px;border-top:1px solid #263348}.hub-page-next{border:1px solid #38bdf8;border-radius:9px;padding:10px 20px;background:linear-gradient(135deg,rgba(56,189,248,.24),rgba(64,217,168,.18));color:#f4f8fd;font:800 13px/1.2 Inter,system-ui,sans-serif;cursor:pointer}.hub-page-next:hover:not(:disabled){transform:translateY(-1px)}.hub-page-next:disabled{opacity:.35;cursor:not-allowed}";
